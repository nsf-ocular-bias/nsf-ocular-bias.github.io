...
function cmBtnClicked() {
    ...
    d3.json("assets/" + this.value + "_UFPR_aligned_aug_gender_finetuned_" + bal + "_0_cm.json").then(function (data) {
...
function cmBtnBalanceClicked(bal, elem) {
    ...
    d3.json("assets/" + model + "_UFPR_aligned_aug_gender_finetuned_" + bal + "_0_cm.json").then(function (data) {
        Plotly.newPlot("cm", data)
    });
...
d3.json("assets/DenseNet121_UFPR_aligned_aug_gender_finetuned_F50M50_0_cm.json").then(function (data) {
    Plotly.newPlot("cm", data)
});


function updateGcChart(tb) {
    d3.csv("assets/ufpr_gender.csv").then(function (data) {
        console.log(data)
...
train_balance = ["F50M50", "F25M75", "F75M25", "F100", "M100"]
...
function updateTbChart(tb, col) {
    d3.csv("assets/ufpr_verification.csv").then(function (data) {
        var data1 = data.filter(function (d) {
...
function plotViz(id, method, tpfn, addBtn = true, model = "EfficientNetB4", cls_="pos") {
    ...
    for (var i = 0; i < 10; ++i) {
        var src = "assets/imgs/" + model + "_UFPR_aligned_aug_gender_finetuned_" + train_balance + "_" + method + "_" + cls_ + "_" + tpfn + "_" + i + ".png"
...
function showAvgModal(id, method) {
    ...
    var activeClass = $(id + '_class .active')[0]
    var activeModel = $(id + '_models .active')[0]
    var src = "assets/imgs/" + activeModel.textContent + "_UFPR_aligned_aug_gender_finetuned_" + train_balance + "_" + method + "_" + activeClass.textContent + "_" + activeTpfn.textContent + "_avg.png"
